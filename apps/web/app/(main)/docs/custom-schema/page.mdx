...
Define a catalog that describes your components and validates props. For custom schemas, use `createCatalog` (the low-level API). If you're using a built-in schema like `@json-render/react`, prefer `defineCatalog` instead — see [Catalog](/docs/catalog).
