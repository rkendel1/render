...
| `TextInput` | `placeholder`, `value` (use `$bind`), `secure`, `keyboardType`, `multiline` | Text input field |
| `Switch` | `checked` (use `$bind`), `label` | Toggle switch |
| `Checkbox` | `checked` (use `$bind`), `label` | Checkbox with label |
| `Slider` | `value` (use `$bind`), `min`, `max`, `step` | Range slider |
| `SearchBar` | `placeholder`, `value` (use `$bind`) | Search input |
